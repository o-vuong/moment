  ...
  render() {
    ...
    const providerList = this.props.providers.map(provider => (
      <tr className="AttrInfo" key={provider.id}>
        ...
        <td>
          <Button bsstyle="link" onClick={() => this.props.editProv(provider)}>
            Edit
          </Button>
        </td>
        <td>
          <Button
            bsstyle="link"
            onClick={() => this.props.deleteProv(provider.id)}
          >
            Delete
          </Button>
        </td>
    ...
    return (
      <tbody>{providerList.length === 0 ? emptyMessage : providerList}</tbody>
    );
  }
}
