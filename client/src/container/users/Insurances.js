  ...
  render() {
    ...
    const insuranceList = this.props.insurances.map(insurance => (
      <tr className="AttrInfo" key={insurance.id}>
        ...
        <td>
          <Button bsstyle="link" onClick={() => this.props.editIns(insurance)}>
            Edit
          </Button>
        </td>
        <td>
          <Button
            bsstyle="link"
            onClick={() => this.props.deleteIns(insurance.id)}
          >
            Delete
          </Button>
        </td>
    ...
    return (
      <tbody>{insuranceList.length === 0 ? emptyMessage : insuranceList}</tbody>
    );
  }
}
